...
class Graph {
 public:
  ...
  std::string GetStr() const {
    std::string ret = "{g, " + std::to_string(id_) + "," + std::to_string(support_);
 ...
 public:
  void Run(TaskGraph* tg, const std::shared_ptr<Properties>& config) const override {
    ...
    auto graph = TextSourceDataset(input, tg, n_partitions)
                     ...
                     .PartitionBy([](const Vertex& v) { return 0; }, 1);
    ...
    start_candidates_label.ApplyRead([](const DatasetPartition<std::pair<LabelType, int>>& data) {
      for (const auto& p : data) {
        LOG(INFO) << "start_candidates_label: " << p.first << " , " << p.second;
    ...
    auto start_candidates = (*shared_graph).SharedDataMapPartitionWith(&start_candidates_label, get_graphs_with_vertex_label);
    ...
    auto extend_subgraph = [](const DatasetPartition<Graph>& src_graph, const DatasetPartition<Graph>& subgraphs) {
      ...
      //   for (const auto& subgraph : subgraphs) {
      //     extended_subgraphs.push_back(subgraph);
      //   }
